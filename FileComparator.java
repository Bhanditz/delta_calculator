...
public class FileComparator{
   //public static File diffFile;
    ...
    public static void compareFiles (String pathOne, String fileOne, String pathTwo, String fileTwo) throws IOException, InterruptedException
    {
        ...
        if (fileTwo.contains("meta"))
        //send the updated meta file to the new DWCA
        {
            File metaFile=new File(DWCA_Updates+"/"+fileTwo);
            BufferedReader bufferedReader= new BufferedReader(new FileReader(pathTwo+"/"+fileTwo));
            BufferedWriter bufferedWriter= new BufferedWriter(new FileWriter(metaFile));
        ...
        File check=new File(pathOne+"/"+fileOne);
        if (check.isDirectory())
            return;
        // the file is being reharvested
        //set the command to sort the two file versions in-place
        fileComparator.executeCommand("sort "+"-c "+pathOne+"/"+fileOne,pathOne+"/"+fileOne);
        fileComparator.executeCommand("sort "+"-c "+pathTwo+"/"+fileTwo,pathTwo+"/"+fileTwo);
    ...
    public void executeCommand(String command, String file) throws IOException, InterruptedException {
        ...
        InputStream commandInputStream=p.getInputStream();
        if(command.contains("sdiff"))
        {
            if(!outputFile.exists())
                outputFile.createNewFile();
            defineModificationType(outputFile,mode.reharvest,commandInputStream);
            return;
        }
